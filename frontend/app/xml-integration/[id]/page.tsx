...
import api from "../../src/services/api"; // Değiştirildi: ../../../ -> ../../
import { useRouter, useParams } from "next/navigation";
import { Card, CardHeader, CardTitle, CardContent } from "../../components/ui/Card";
import { ArrowLeft, Check, UploadCloud, Link as LinkIcon, AlertCircle, Save } from "lucide-react";

export default function EditXmlIntegrationPage() {
  ...
  useEffect(() => {
    // Mevcut entegrasyon bilgilerini çek
    if (id) {
      setFetching(true);
      api.get(`/xml/integrations/${id}`)
        .then(res => {
          const data = res.data;
          setUrl(data.xml_url);
          setMapping(data.field_mapping || {});
          
          // URL geldikten sonra otomatik analiz yapalım ki preview görünsün
          if (data.xml_url) {
             fetchPreview(data.xml_url);
          }
        })
        .catch(err => {
          console.error("Entegrasyon bilgileri alınamadı", err);
          setError("Entegrasyon bulunamadı.");
        })
        .finally(() => setFetching(false));
    }
  }, [id]);

  const fetchPreview = async (xmlUrl: string) => {
    setLoading(true);
    setError("");
    try {
      const response = await api.post("/xml/preview", { url: xmlUrl });
      setPreviewData(response.data);
    } catch (err: any) {
      console.error("Önizleme hatası:", err);
      // Sessiz kalabilir veya hata gösterebiliriz, 
      // ama edit sayfasında mevcut mapping'i göstermek daha önemli.
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      await api.put(`/xml/integrations/${id}`, payload);
      alert("Entegrasyon güncellendi!");
      router.push('/xml-integration'); 
      
    } catch (err: any) {
      console.error(err);
      alert("Hata: " + (err.response?.data?.message || "Güncellenemedi."));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="max-w-4xl mx-auto animate-in fade-in zoom-in duration-300 space-y-6">
      ...
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
            <h1 className="text-2xl font-bold text-white tracking-tight">Entegrasyonu Düzenle</h1>
            <p className="text-slate-400 text-sm">#{id} numaralı entegrasyonu düzenliyorsunuz.</p>
        </div>
      </div>
         ...
         <CardContent>
            <div className="flex gap-3">
              ...
              >
                ...
                {loading ? "Analiz..." : "Yeniden Analiz Et"}
              </button>
            </div>
            {error && (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                 ...
                 <div className="space-y-4">
                    {[
                      ...
                      { key: 'category', label: 'Kategori / Koleksiyon', req: false }
                    ].map((field) => (
                       <div key={field.key}>
